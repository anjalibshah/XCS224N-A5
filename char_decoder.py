...
class CharDecoder(nn.Module):
    def __init__(self, hidden_size, char_embedding_size=50, target_vocab=None):
        ...
        self.char_output_projection = None
        self.decoderCharEmb = None
    ...
    def forward(self, input, dec_hidden=None):
        ...
        outputs, dec_hidden = self.charDecoder(x_emb, dec_hidden)
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        input_char_seq = char_sequence[:-1, :].contiguous()
        target_char_seq = char_sequence[1:, :].contiguous()
        scores, dec_hidden = self.forward(input_char_seq, dec_hidden)
        loss = nn.CrossEntropyLoss(ignore_index=self.target_vocab.char2id['<pad>'], reduction='sum')
        cross_entropy_loss = loss(scores.contiguous().permute(1,2,0), target_char_seq.transpose(1,0))
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        output_words = torch.cat(output_words).contiguous().transpose(1,0)

        for i,x in enumerate(output_words):
            word = ''
            id_list = x.tolist()
            for item in id_list:
                if item == end_idx:
                    break
                word += self.target_vocab.id2char[item]
            decodedWords.append(word)
        #print(len(decodedWords))
        return decodedWords
        ### END YOUR CODE
