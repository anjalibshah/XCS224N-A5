...
def question_1c_sanity_check(model):
    ...
    input_t = vocab.to_input_tensor_char(sentences, model.device)
    assert list(input_t.shape) == [6, 4, 21], "Tensor shape is incorrect: it should be: \n {} but is:\n{}".format([6,4,21], list(input_t.shape))
    
    print("Sanity Check Passed for Question 1c: Input Tensor!")
    print("-" * 80)


def question_1d_sanity_check(model):
    """ Sanity check for highway network.
    """
    print("-" * 80)
    print("Running Sanity Check for Question 1d: Highway Tensor")
    print("-" * 80)
    highway = Highway(EMBED_SIZE, DROPOUT_RATE)

    print("Running test on highway network outputs")
    
    x_conv_out = torch.randn(4,3)
    x_word_emb = highway.forward(x_conv_out)
    assert list(x_word_emb.shape) == [4, 3], "Tensor shape is incorrect: it should be: \n {} but is:\n{}".format([4,3], list(x_word_emb.shape))

    print("Sanity Check Passed for Question 1d: Highway Tensor!")
    print("-" * 80)
...
def question_1e_sanity_check(model):
    ...
    """
    print("-" * 80)
    print("Running Sanity Check for Question 1e: Convolutional Encoder")
    print("-" * 80)
    
    cnn = CNN(50, 21, 5, 3)

    print("Running test on convolutional encoder network")
    
    x_reshaped = torch.randn(4, 50, 21)
    x_conv_out = cnn.forward(x_reshaped)
    assert list(x_conv_out.shape) == [4, 3], "Tensor shape is incorrect: it should be: \n {} but is:\n{}".format([4, 3], list(x_conv_out.shape))

    print("Sanity Check Passed for Question 1e: Convolutional Encoder!")
    print("-" * 80)
...
def main():
    ...
    if args['1a']:
        ...
        question_1c_sanity_check(model)
    elif args['1d']:
        question_1d_sanity_check(model)
    elif args['1e']:
